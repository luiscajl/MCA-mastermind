...
public class YesNoDialog extends WithConsoleModel {
    ...
    private static final String QUESTION = "? (" + YesNoDialog.AFIRMATIVE + "/"
            + YesNoDialog.NEGATIVE + "): ";

    private static final String MESSAGE = "The value must be '"
            + YesNoDialog.AFIRMATIVE + "' or '" + YesNoDialog.NEGATIVE + "'";
    ...
    public boolean read() {
        ...
        do {
            ...
            ok = YesNoDialog.isAfirmative(answer)
                    || YesNoDialog.isNegative(answer);
            if (!ok) {
