...
class GameView extends JFrame {
    ...
    void interact(StartController startController) {
        ...
        this.getContentPane().add(this.secretCombinationView,
                new Constraints(0, 0, 3, 1));
        this.proposedCombinationsView = new ProposedCombinationsView();
        this.getContentPane().add(this.proposedCombinationsView,
                new Constraints(0, 1, 3, 10));
        this.proposalCombinationView = new ProposalCombinationView(
                this.getRootPane());
        this.getContentPane().add(this.proposalCombinationView,
                new Constraints(0, 11, 3, 1));
        this.setVisible(true);
    }
    ...
    void interact(ProposalController proposalController) {
        ...
        do {
            List<Color> colors = new ProposedCombinationView()
                    .read(this.proposalCombinationView.getCharacters());
            error = proposalController.addProposedCombination(colors);
            if (error != null
                    && this.proposalCombinationView.getCharacters() != "") {
                JOptionPane.showMessageDialog(null,
                        new ErrorView(error).getMessage(), "ERROR",
    ...
    private void drawGameOver(ProposalController proposalController) {
        if (proposalController.isWinner() || proposalController.isLooser()) {
            ...
            JOptionPane.showMessageDialog(null, message, GameView.GAME_OVER,
                    JOptionPane.WARNING_MESSAGE);
        }
    }
